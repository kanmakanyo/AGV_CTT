...
while True:
    ...
    # dt_sim = 0.01
    V_AV, cs_steer, rho_, alpha_, errx_ = CTT.calculate_point_stabilization(X_pos_t, Y_pos_t, yaw_t, X_target, Y_target, yaw_target, k_rho, k_alpha, l_t, kv_alpha)
    ...
    # update 10/05/22, add the k_rho gain update
    if rho_ <= 10: 
        k_rho = 0.4
...
if cs_steer is not 0: #bring the steering angle to 0, with some steer angle rate value
    steer_inc_ = cs_steer/10 #increase in 10 small step
    V_AV = 0 #move only the steering angle

for i in range(10): 
    cs_steer += steer_inc_*-1
    X_pos_t, Y_pos_t, yaw_t = CTT.update_AV_position(X_pos_t, Y_pos_t, V_AV, yaw_t, cs_steer, dt_sim, l_t)
