...
def callback_dbwsub(msg):
    ...
    deg_steer = ((volt_f_steer - 2.6603)/0.013) + 6.5 #- 2.5 #in degree
    # if control_mode == 1:
...
def callback_lidar_pos(msg):
    ...
    y_est = y_lidar #+ 1 * np.sin(yaw_est) - (0) * np.cos(yaw_est)
...
while not rospy.is_shutdown():
    ...
    pub_msg.pose.covariance[4] = x_est#temp_msg['x_est'] #sensor_data['x_rear'] #temp_msg['x_est']
    pub_msg.pose.covariance[5] = y_est#temp_msg['y_est'] #sensor_data['y_rear'] 
    pub_msg.pose.covariance[6] = temp_msg['v_est']
    pub_msg.pose.covariance[7] = yaw_est#temp_msg['yaw_trailer']
    pub_msg.pose.covariance[8] = yaw_est + rad_steer#temp_msg['yaw_head']
